...
png(filename = "plot4.png")  # png() creates by default 480x480 pixel
# Set 2x2 grid, keep previous par setting.
opar <- par(mfrow = c(2, 2))
plot(dt[, list(date.time, Global_active_power)], type = "l", xlab = "", ylab = "Global Active Power")
plot(dt[, list(date.time, Voltage)], type = "l", xlab = "datetime")
...
plot(dt[, list(date.time, Global_reactive_power)], type = "l", xlab = "datetime")
dev.off() # png off
par <- opar # restore previous par
